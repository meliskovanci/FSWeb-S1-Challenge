...
for(let i=0 ; i<fenomen.length ;i++) {
  copyProfileName.push(fenomen[i].profile);
}  
...
function fenomenSil(array,indexSayisi) {
  let newArray = [...array];
  newArray.splice(indexSayisi,1);
  return newArray;
}
...
function fenomenEkle(array,gelenSayi,gelenProfile,gelenFollowers,gelenPosts,gelenPlatforms) {
  
  let newArr = [...array];

  const newObje = {};

  newObje.number = gelenSayi;
  newObje.profile = gelenProfile;
  newObje.followers = gelenFollowers;
  newObje.posts = gelenPosts;
  newObje.platform = gelenPlatforms;
...
function platformaGoreCokGonderiYapanFenomen(dizi,x){
  let enFenomen = "";
  let maxPost = 0 ;
  for (let i=0 ; i < fenomenler.length ;i++){
    if (dizi[i].platform == x &&
       dizi[i].posts!="NA" &&
       dizi[i].posts>maxPost){

        enFenomen = dizi[i].profile;
